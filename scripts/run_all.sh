...
# python3 -m scripts.generate_malay -in ${MALAY_CSV_PATH} -out ${MALAY_OUT_DIR} -l 1 -n malay
...
python3 -m scripts.generate_welsh -in ${WELSH_CSV_PATH} -out ${WELSH_OUT_DIR} -l 1 -n welsh
...
# python3 -m scripts.generate_french -in ${FRENCH_CSV_PATH} -out ${FRENCH_OUT_DIR} -l 1 -n french
...
# python3 -m scripts.generate_avar -in ${AVAR_CSV_PATH} -out ${AVAR_OUT_DIR} -l 1 -n avar
